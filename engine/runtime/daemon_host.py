...
def init_daemon_dependencies(
    ...
    *,
    ...
    host._circuit_breakers: Dict[str, Any] = {}  # per-novel 熔断器（key=novel_id）
...
class DaemonHostMixin:
    ...
    def _get_circuit_breaker(self, novel_id: str):
        """获取指定小说的独立熔断器（不存在则创建）

        每本小说拥有独立的熔断器实例，一本小说的 API 故障不会导致其他小说一起熔断。
        """
