...
#
#  使用示例:
#    ./run.sh start              # 启动前后端（Docker）
#    ./run.sh dev start          # 启动开发环境（本地直启）
#    ./run.sh dev start backend  # 仅启动后端开发服务
#    ./run.sh stop frontend      # 仅停止前端（保持后端运行）
#    ./run.sh status             # 查看生产服务状态
#    ./run.sh dev status         # 查看开发环境状态
#    ./run.sh logs backend       # 跟踪生产容器日志
#    ./run.sh dev logs backend   # 跟踪开发后端日志
# ============================================================
...
使用示例:
  ...
  ./run.sh dev start          # 启动开发环境（本地直启）
  ./run.sh dev start backend  # 仅启动后端开发服务
  ./run.sh dev start frontend # 仅启动前端开发服务
  ./run.sh dev restart        # 重启全部开发服务
  ./run.sh dev restart backend # 仅重启后端开发服务
  ./run.sh dev start -log     # 启动并持续监听日志（Ctrl+C 退出）
  ./run.sh dev stop backend   # 仅停止后端开发服务
  ./run.sh stop frontend      # 仅停止生产前端（保持后端运行）
  ./run.sh status             # 查看生产服务状态
  ./run.sh dev status         # 查看开发环境状态
  ./run.sh logs backend       # 跟踪生产容器日志
  ./run.sh dev logs backend   # 跟踪开发后端日志
...
# ── update: git pull origin <当前分支> → 重新构建镜像 → 重启 ──
...
cmd_update() {
    ...
    if [ -z "$current_branch" ] || [ "$current_branch" = "HEAD" ]; then
        _warn "无法检测当前分支（可能处于 detached HEAD），尝试 git pull…"
        if ! git pull --ff-only; then
            _warn "git pull 未成功，继续构建…"
        else
            _ok "git pull 完成"
        fi
    else
        _info "当前分支: ${current_branch}"
        if git pull origin "$current_branch" --ff-only; then
            _ok "git pull origin ${current_branch} 完成"
        else
            _warn "git pull origin ${current_branch} 未成功（可能是本地有修改或网络问题），继续构建…"
        fi
    fi
